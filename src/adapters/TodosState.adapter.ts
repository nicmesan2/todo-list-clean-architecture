...
export const useTodoStateMemoryAdapter: () => TodoStateService = (initTodos: Todos = {}) => {
    ...
    const selectAll = () => Promise.resolve(todos);
...
export const useTodoStateReduxAdapter: () => TodoStateService = (initTodos: Todos = {}) => {
    type RootState = ReturnType<typeof store.getState>;

    const todosAdapter = createEntityAdapter<Todo>(initTodos);
    ...
    const todosSelector = todosAdapter.getSelectors<RootState>((state) => state.todos);

    const selectAll = () => todosSelector.selectEntities(store.getState()) as Todos;
