...
export const TodoSWRProvider: React.FC<TodoSWRStoreProviderProps> = ({ children, todoStateService }) => {
    ...
    const addTodo = useCallback(
        async (todoDescription: string) => {
            mutate(CACHE_KEY, () => useCases.addNewTodo(todoDescription, todoStateService), {
    ...
    const moveToPreviousState = useCallback(
        async (todo: Todo) => {
            mutate(CACHE_KEY, () => useCases.changeToPreviousState(todo, todoStateService), {
                revalidate: true,
                populateCache: false,
                optimisticData: (currentData: Todos) => {
    ...
    const moveToNextState = useCallback(
        async (todo: Todo) => {
            mutate(CACHE_KEY, () => useCases.changeToNextState(todo, todoStateService), {
                revalidate: true,
                populateCache: false,
                optimisticData: (currentData: Todos) => {
    ...
    return <TodoSWRContext.Provider value={values}>{children}</TodoSWRContext.Provider>;
};

const TodoSWRContext = React.createContext<any>({});

export const useTodoSWRImplementation = () => useContext(TodoSWRContext);
